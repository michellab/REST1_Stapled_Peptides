...
def get_protein_atom(input_parm):
    ...
    solventype= ['OW','HW', 'CS', 'F', 'OF', 'HO', 'HT',  'Cl-','Na+', 'K+']
    solvent=['WAT', 'SOL' , 'TFE','Cl-','Na+', 'K+' ]
    # protein atomtypes
    proteic_atom = []
    for resid in input_parm.residues:
        for atom in resid.atoms:
                ...
                if atom.type.isupper() and atom.idx not in proteic_atom and atom.type not in solvent:
...
def scale_protein_dihedral(input_parm,  sfactor, verbose=False):
    ...
    if verbose : print(str(len(protdihtype))+ ' protein dihedral types to scale on ' +str(len( input_parm.dihedral_types)))
...
if __name__ == "__main__":
    ...
    for rep in np.arange(nreps):
        ...
        new_parm_charge = scale_protein_charges(new_parm_LJ, sfactor, verbose)
        if parmed.tools.actions.netCharge(new_parm_charge).execute() !=0 :

            new_parm_charge_reweigthed =neutralize_protein_charges(new_parm_charge,  verbose)
